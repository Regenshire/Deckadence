(function () {
  ...
  function getFilenameFromContentDisposition(
    ...
    if (utfMatch && utfMatch[1]) {
      try {
        return (
          decodeURIComponent(utfMatch[1].replace(/"/g, "").trim()) || fallback
        );
      } catch (error) {
  ...
  function initPrintExportModal(config) {
    ...
    const openButton = getElement(
      config.openButtonId || "deckbuilderOpenPrintExportButton",
    );
    ...
    const subtitle = modal
      ...
      : null;
    const progressUrl = modal
      ? String(modal.dataset.progressUrl || "").trim()
      : "";
    ...
    async function pollProgress(progressId) {
      if (!progressUrl || !progressId || progressPollInFlight) {
        return;
      }
      ...
      try {
        ...
        updateStatus(
          ...
          payload.title || "Generating file",
          payload.message || "Working...",
        );
      } catch (error) {
        // Progress is supplemental. Never fail the job
    ...
    function rememberPrintExportSettings() {
      if (!form) {
        return;
      }

      [
        "show_print_export_labels",
        "silhouette_registration_marks",
        "pdf_cutting_guides",
        "include_pack_label_cards",
      ...
      form
        .querySelectorAll('input[type="radio"][name="label_text_mode"]')
    ...
    function setExtraFormFields(extraFormFields) {
      activeExtraFormFields =
        extraFormFields && typeof extraFormFields === "object"
          ? extraFormFields
          : {};
    }
    ...
    function appendExtraFormFields(formData) {
      ...
      Object.keys(activeExtraFormFields).forEach(function (fieldName) {
        ...
        const fieldValues = Array.isArray(rawValue) ? rawValue : [rawValue];
    ...
    function setButtonsWorking(isWorking) {
      [printButton, exportZipButton, openButton].forEach(function (button) {
        ...
        button.disabled =
          Boolean(isWorking) || button.dataset.originalDisabled === "1";
    ...
    async function runPrintExport(actionType) {
      ...
      formData.set("print_export_action", isExport ? "export" : "print");

      formData.set("print_export_progress_id", progressId);

      appendExtraFormFields(formData);
      rememberPrintExportSettings();
