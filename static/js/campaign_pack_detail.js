(function () {
  ...
  const zoomableImages = Array.from(
    document.querySelectorAll(".campaign-pack-detail-zoomable"),
  );
  const changePrintingButtons = Array.from(
    document.querySelectorAll(".campaign-pack-change-printing-button"),
  );
  const changePrintingOverlay = document.getElementById(
    "campaignPackChangePrintingOverlay",
  );
  const changePrintingCloseButton = document.getElementById(
    "campaignPackChangePrintingCloseButton",
  );
  const changePrintingSubtitle = document.getElementById(
    "campaignPackChangePrintingSubtitle",
  );
  const changePrintingStatus = document.getElementById(
    "campaignPackChangePrintingStatus",
  );
  const changePrintingResults = document.getElementById(
    "campaignPackChangePrintingResults",
  );
  ...
  function getCampaignPackBatchUpscaleCardUuids() {
    ...
    Array.from(
      document.querySelectorAll(".chaos-pack-view-card[data-card-uuid]"),
  ...
  function setChangePrintingStatus(messageText, isError) {
    ...
    changePrintingStatus.classList.toggle(
  ...
  function openChangePrintingModal(button) {
    ...
    if (
      ...
      !activeChangePrintingState.printingOptionsUrl ||
      !activeChangePrintingState.updateUrl
    ) {
      return;
    }

    if (changePrintingSubtitle) {
      changePrintingSubtitle.textContent =
  ...
  function renderChangePrintingOptions(cards) {
    ...
    const sortedCards = cards.slice().sort(function (cardA, cardB) {
      const cardACurrent =
        cardA.card_uuid === activeChangePrintingState.currentCardUuid;
      const cardBCurrent =
        cardB.card_uuid === activeChangePrintingState.currentCardUuid;
      ...
      return (
        getCampaignPackReleaseDateSortValue(cardB) -
        getCampaignPackReleaseDateSortValue(cardA)
      );
    });
  ...
  async function loadChangePrintingOptions() {
    ...
    try {
      ...
      setChangePrintingStatus(
        "Loaded " + (payload.results || []).length + " printing(s).",
        false,
      );
    } catch (error) {
      console.error(error);
      setChangePrintingStatus(
        error.message || "Could not load printings.",
        true,
      );
    }
  }
  ...
  async function changeCampaignPackCardPrinting(card, button) {
    ...
    try {
      ...
      setChangePrintingStatus(
        error.message || "Failed to change printing.",
        true,
      );
    }
  }
    ...
    matchingCards.forEach(function (cardElement) {
      ...
      if (isFoil) {
        if (!foilBadge) {
          ...
          badgeWrap.className =
            "chaos-pack-view-badges campaign-pack-foil-badge";
          ...
          const existingBadges = infoWrap
            ? infoWrap.querySelector(".chaos-pack-view-badges")
            : null;
  ...
  function setCampaignPackImageFace(
    imageElement,
    metadata,
    faceName,
    flipButton,
  ...
  function flipCampaignPackImage(imageElement, flipButton) {
    ...
    if (imageElement === zoomImage && activeZoomSourceImage) {
      ...
      setCampaignPackImageFace(
        activeZoomSourceImage,
        metadata,
        nextFace,
        sourceButton,
      );
  ...
  if (zoomOverlay && zoomBackdrop && zoomImage && zoomableImages.length) {
    function openZoom(imageElement) {
      ...
      const imageSrc =
        currentFace === "back" && metadata
          ? metadata.back_src
          : imageElement.src ||
            imageElement.getAttribute("data-zoom-src") ||
            "";
      const imageAlt =
        currentFace === "back" && metadata
          ? metadata.back_alt
          : imageElement.alt ||
            imageElement.getAttribute("data-zoom-alt") ||
            "";
      ...
      zoomImage.dataset.cardUuid =
        getCardUuidForCampaignPackImage(imageElement);
      ...
      if (zoomContent) {
        ...
        if (metadata && metadata.is_dual_faced && metadata.back_src) {
          ...
          if (activeZoomFlipButton) {
            if (currentFace === "back") {
              activeZoomFlipButton.classList.add(
                "campaign-pack-flip-button-flipped",
              );
              activeZoomFlipButton.setAttribute(
                "aria-label",
                "Show front face",
              );
    ...
    zoomImage.addEventListener("click", function (event) {
      if (
        ...
        event.target.closest(".campaign-pack-flip-button")
      ) {
        return;
      }
    ...
    document.addEventListener("keydown", function (event) {
      ...
      if (
        changePrintingOverlay &&
        !changePrintingOverlay.classList.contains("hidden")
      ) {
        closeChangePrintingModal();
        return;
      }
  ...
  const packViewCopyMainButton = document.getElementById(
    "packViewCopyMainButton",
  );
  const packViewCopyMenuButton = document.getElementById(
    "packViewCopyMenuButton",
  );
  const packViewCopyMenu = document.getElementById("packViewCopyMenu");
  const packViewCopyFormatButtons = Array.from(
    document.querySelectorAll(".pack-view-copy-format-button"),
  );
  ...
  function togglePackViewCopyMenu() {
    ...
    packViewCopyMenuButton.setAttribute(
      "aria-expanded",
      willOpen ? "true" : "false",
    );
  }
  ...
  function setSelectedPackViewExportFormat(exportFormat) {
    const normalizedFormat = String(exportFormat || "")
      .trim()
      .toLowerCase();
    ...
    packViewCopyFormatButtons.forEach(function (button) {
      button.classList.toggle(
        ...
        (button.getAttribute("data-export-format") || "") ===
          selectedPackViewExportFormat,
      );
    });
  }
  ...
  function setCampaignPackContextActionVisible(menu, actionName, isVisible) {
  ...
  function prepareCampaignPackCardContextMenu(cardElement, menu) {
    const changePrintingButton = cardElement.querySelector(
      ".campaign-pack-change-printing-button",
    );
    const alternateImageButton = cardElement.querySelector(
      ".alternate-image-button",
    );
    const upscaleButton = cardElement.querySelector(
    ...
    setCampaignPackContextActionVisible(
      menu,
      "change-printing",
      Boolean(changePrintingButton),
    );
    setCampaignPackContextActionVisible(menu, "foil", Boolean(foilUpdateUrl));
    setCampaignPackContextActionVisible(
      menu,
      "alternate-image",
    ...
    if (foilMenuItem) {
      foilMenuItem.textContent =
        cardElement.dataset.isFoil === "1" ? "Remove Foil" : "Set Foil";
    }
  }

  async function handleCampaignPackCardContextAction(actionName, cardElement) {
    const changePrintingButton = cardElement.querySelector(
      ".campaign-pack-change-printing-button",
    );
    const alternateImageButton = cardElement.querySelector(
    ...
    const nextIsFoil = cardElement.dataset.isFoil !== "1";
  ...
  function initializeCampaignPackCardContextMenu() {
    if (
      !window.iMomirCardContextMenu ||
    ...
    window.iMomirCardContextMenu.bind({
      menu: "campaignPackCardContextMenu",
      targetSelector: ".chaos-pack-view-card",
      prepare: prepareCampaignPackCardContextMenu,
      onAction: handleCampaignPackCardContextAction,
      onError: function (error) {
        showCampaignPackMessage(error.message || String(error), true);
      },
    });
  }
  ...
  document.addEventListener(
    "DOMContentLoaded",
    initializeCampaignPackCardContextMenu,
    { once: true },
  );
  ...
  if (batchUpscaleButton) {
    batchUpscaleButton.addEventListener("click", function () {
      const cardUuids = getCampaignPackBatchUpscaleCardUuids();
  ...
  if (changePrintingCloseButton) {
    changePrintingCloseButton.addEventListener(
      "click",
      closeChangePrintingModal,
    );
  }
  ...
  packViewCopyFormatButtons.forEach(function (button) {
    button.addEventListener("click", async function () {
      setSelectedPackViewExportFormat(
        button.getAttribute("data-export-format") || "archidekt",
      );
