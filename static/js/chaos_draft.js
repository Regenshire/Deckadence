...
function initializeChaosSpinLogoAnimation(spinCtaButton) {
  const rotor = spinCtaButton.querySelector(".chaos-draft-spin-logo-rotor");
  ...
  function setSpeedFromPointer(event) {
    ...
    const accelerationRadius = radius * (1 + accelerationMarginRatio);
    ...
    const proximity = 1 - distance / accelerationRadius;

    const easedProximity = proximity * proximity * (3 - 2 * proximity);

    targetRotationsPerSecond =
      baseRotationsPerSecond +
      (maxRotationsPerSecond - baseRotationsPerSecond) * easedProximity;
  }
  ...
  function animate(timestamp) {
    ...
    const responseAmount = 1 - Math.exp(-speedResponse * elapsedSeconds);

    currentRotationsPerSecond +=
      (targetRotationsPerSecond - currentRotationsPerSecond) * responseAmount;

    currentAngle =
      (currentAngle + currentRotationsPerSecond * 360 * elapsedSeconds) % 360;
  ...
  document.addEventListener("pointermove", setSpeedFromPointer, {
    passive: true,
  });

  document.documentElement.addEventListener("pointerleave", resetPointerSpeed);

  window.addEventListener("blur", resetPointerSpeed);
...
function initializeChaosDraftPage() {
  ...
  const chaosSpinUrl =
    chaosDraftScreen.dataset.chaosSpinUrl || "/chaos-draft/spin";
  const chaosOpenUrl =
    chaosDraftScreen.dataset.chaosOpenUrl || "/chaos-draft/open";
  const chaosViewDataUrl =
    chaosDraftScreen.dataset.chaosViewDataUrl || "/chaos-draft/view-data";
  const chaosExportUrl =
    chaosDraftScreen.dataset.chaosExportUrl || "/chaos-draft/export";
  const chaosExportZipUrl =
    chaosDraftScreen.dataset.chaosExportZipUrl || "/chaos-draft/export-zip";
  const cardFaceDataUrl =
    chaosDraftScreen.dataset.cardFaceDataUrl || "/card-face-data";
  ...
  const inlineViewUpscaleButton = document.getElementById(
    "chaosPackInlineUpscaleButton",
  );
  ...
  const printExportPrintButton = document.getElementById(
    "printExportPrintButton",
  );
  ...
  const chaosExportFormat = chaosDraftScreen
    ? (
        chaosDraftScreen.getAttribute("data-chaos-export-format") || "none"
      ).toLowerCase()
    : "none";
  ...
  initializeChaosSpinLogoAnimation(spinCtaButton);
  ...
  const jackpotBoosterTypes = new Set(["collector", "vip", "premium"]);
  ...
  const badPackSetCodes = new Set(["HML", "PCY", "FEM", "DRK", "CHR"]);
  ...
  function setInlinePackImageFace(
    imageElement,
    metadata,
    faceName,
    flipButton,
    ...
    const targetSrc =
      targetFace === "back" ? metadata.back_src : metadata.front_src;
    const targetAlt =
      targetFace === "back" ? metadata.back_alt : metadata.front_alt;
  ...
  function flipInlinePackImage(imageElement, flipButton) {
    ...
    const currentFace =
      imageElement.dataset.currentFace === "back" ? "back" : "front";
    ...
    if (imageElement === packZoomImage && activeInlineZoomSourceImage) {
      ...
      setInlinePackImageFace(
        activeInlineZoomSourceImage,
        metadata,
        nextFace,
        sourceButton,
      );
  ...
  function addInlinePackFlipButtonToImage(imageElement) {
    ...
    if (
      !imageWrap ||
      imageWrap.querySelector(".chaos-pack-inline-flip-button")
    ) {
      return;
    }

    imageWrap.classList.add("chaos-pack-inline-flip-host");
    imageElement.dataset.currentFace =
  ...
  function openPackZoom(imageElement) {
    ...
    if (zoomContent) {
      ...
      if (metadata && metadata.is_dual_faced && metadata.back_src) {
        ...
        if (activeInlineZoomFlipButton) {
          updateInlinePackFlipButtonFace(
            activeInlineZoomFlipButton,
            currentFace,
          );
  ...
  function buildPackBadgeMarkup(cardData) {
    ...
    if (
      ...
      Number.isFinite(Number(cardData.price))
    ) {
      const numericPrice = Number(cardData.price);
      const priceClass =
        numericPrice > 2 ? " chaos-pack-inline-price-high" : "";
      badges.push(
        `<span class="chaos-pack-inline-price${priceClass}">${formatPackCardPrice(numericPrice)}</span>`,
      );
    }

    const specialBadges = Array.isArray(cardData.special_badges)
      ? cardData.special_badges
      : [];
  ...
  function buildInlinePackUpscaleButtonMarkup(cardData) {
    const cardUuid = String(cardData.card_uuid || "").trim();

    const upscaleControlUrl = String(cardData.upscale_control_url || "").trim();

    if (!cardUuid || !upscaleControlUrl) {
      return "";
    }

    const activeClass = cardData.has_upscaled_image
    ...
    return `
            <button
                type="button"
                class="chaos-pack-inline-upscale-button imomir-upscale-card-button${activeClass}"
                data-card-uuid="${cardUuid}"
                data-upscale-control-url="${upscaleControlUrl}"
                title="Upscale Image"
  ...
  function getInlinePackBatchUpscaleCardUuids() {
    if (!inlineViewGrid) {
      return [];
    }
    ...
    inlineViewGrid
      .querySelectorAll(".chaos-pack-inline-card[data-card-uuid]")
      .forEach(function (cardElement) {
        const cardUuid = String(cardElement.dataset.cardUuid || "").trim();

        if (!cardUuid || seenCardUuids.has(cardUuid)) {
          return;
        }

        seenCardUuids.add(cardUuid);
        cardUuids.push(cardUuid);
      });
  ...
  function refreshInlinePackCardImages(cardUuid) {
    if (!cardUuid) {
      return;
    }
    ...
    document
      .querySelectorAll(
        '.chaos-pack-inline-image[data-card-uuid="' +
          CSS.escape(cardUuid) +
          '"]',
      )
      .forEach(function (imageElement) {
        try {
          ...
          imageUrl.searchParams.set("_image_refresh", refreshToken);
    ...
    if (
      packZoomImage &&
      getInlinePackCardUuid(packZoomImage) === cardUuid &&
      packZoomOverlay &&
      !packZoomOverlay.classList.contains("hidden")
    ) {
      try {
        const zoomUrl = new URL(packZoomImage.src, window.location.href);

        zoomUrl.searchParams.set("_image_refresh", refreshToken);

        packZoomImage.src = zoomUrl.toString();
      } catch (error) {}
    }
  }

  function setInlinePackUpscaleIndicatorActive(cardUuid, isActive) {
    if (!cardUuid) {
      return;
    }
    ...
    document
      .querySelectorAll(
        '.chaos-pack-inline-upscale-button[data-card-uuid="' +
          CSS.escape(cardUuid) +
          '"]',
      )
  ...
  async function refreshInlinePackUpscaleResults() {
    if (!inlineViewGrid) {
      return;
    }
    ...
    if (!response.ok || !payload.ok) {
      throw new Error(
        payload.message || "Failed to refresh Upscaled card images.",
      );
    }
    ...
    cards.forEach(function (cardData) {
      ...
      const visibleCard = inlineViewGrid.querySelector(
        '.chaos-pack-inline-card[data-card-uuid="' +
          CSS.escape(cardUuid) +
          '"]',
      );
      ...
      refreshInlinePackCardImages(cardUuid);
      setInlinePackUpscaleIndicatorActive(
        cardUuid,
        Boolean(cardData.has_upscaled_image),
      );
    });
  }
  ...
  async function renderInlinePackView(payload) {
    ...
    if (inlineViewTitle) {
      inlineViewTitle.textContent =
        payload.pack_display_name || "Pack contents";
    }
    ...
    cards.forEach(function (cardData) {
      ...
      const upscaleButtonMarkup = buildInlinePackUpscaleButtonMarkup(cardData);
      ...
      cardElement.innerHTML = `
                ...
                <div class="chaos-pack-inline-info">
                    <div class="chaos-pack-inline-name-row">
    ...
    if (inlineViewUpscaleButton) {
      inlineViewUpscaleButton.disabled = cards.length === 0;
    }
  ...
  function playRouletteTick() {
    ...
    playTone(484, 0.115, "square", 0.012, 0.0);
    playTone(968, 0.07, "square", 0.0035, 0.0);
  }
  ...
  function playJackpotSound() {
    ...
    // Opening fanfare
    playTone(784, 0.16, "sawtooth", v, 0.0); // G5
    ...
    playTone(1568, 0.2, "sawtooth", v + 0.004, 0.72); // G6
    playTone(1976, 0.24, "sawtooth", v + 0.006, 0.88); // B6

    // Final victory hold
    playTone(2350, 0.95, "sawtooth", v + 0.01, 1.06); // D7

    // Bright brass shimmer
    playTone(4700, 0.72, "triangle", 0.012, 1.08);
    playTone(3136, 0.82, "triangle", 0.01, 1.06);
  }
  ...
  function playBadPackSound() {
    ...
    // First horn (shorter)
    playDecayingTone(370, 0.55, "sawtooth", v, 0.0); // F#4-ish
    playDecayingTone(740, 0.4, "triangle", 0.01, 0.0); // harmonic layer

    // Second horn (longer, lower, sadder)
    playDecayingTone(277, 0.95, "sawtooth", v + 0.004, 0.38); // C#4-ish
    playDecayingTone(554, 0.7, "triangle", 0.01, 0.38); // harmonic
  }
  ...
  function isBigWinPack(packInfo) {
    ...
    const boosterName = String(packInfo.booster_name || "")
      .trim()
      .toLowerCase();
    const setCode = String(packInfo.set_code || "")
      .trim()
      .toUpperCase();
  ...
  function applySelectedExportActionUi() {
    ...
    if (exportMainButton) {
      exportMainButton.textContent =
        selectedExportAction === "save" ? "Save" : "Copy";
    }
  }
  ...
  function setSavePackButtonState(isBusy, savedText) {
    ...
    savePackButton.disabled =
      Boolean(isBusy) || !currentSpinResult || currentPackSavedToDb;
  ...
  async function saveCurrentPackToDb(showAlertOnSuccess) {
    ...
    try {
      ...
      if (showAlertOnSuccess) {
        window.alert(
          payload.message || "Pack saved to the Pack Tracking Database.",
        );
      }
  ...
  function renderSpinnerCards(spinResult, repeatCount) {
    ...
    const repeatedSequence = buildRepeatedPackSequence(
      displayPacks,
      repeatCount,
    );
  ...
  function getCenteredTranslateForCard(cardElement) {
    const spinnerWindow = spinnerShell.querySelector(
  ...
  function animateTrackToTarget(finalAbsoluteIndex, visiblePackCount) {
    ...
    const allCards = spinnerTrack.querySelectorAll(".chaos-pack-card");
    const finalCard = spinnerTrack.querySelector(
      `[data-chaos-card-index="${finalAbsoluteIndex}"]`,
    );
    ...
    function snapToCenter() {
      ...
      window.setTimeout(function () {
        const winningVisualClass =
          getWinningVisualClassForPack(currentWinningPack);
    ...
    function step(timestamp) {
      ...
      const currentTranslate =
        startTranslate + (approachTranslate - startTranslate) * easedProgress;
  ...
  function runSpinAnimation(spinResult) {
    ...
    const finalAbsoluteIndex =
      winningRepeatIndex * displayPacks.length + winningStopIndex;

    if (
      !repeatedSequence.length ||
      finalAbsoluteIndex < 0 ||
      finalAbsoluteIndex >= repeatedSequence.length
    ) {
      stopRouletteTicks();
      animationInProgress = false;
      setButtonsForIdle();
      message.classList.remove("hidden");
      message.textContent =
        "Chaos Draft spin failed to resolve the winning pack.";
      return;
    }
  ...
  async function runSpin() {
    ...
    window.setTimeout(async function () {
      try {
        ...
        if (!response.ok || !payload.ok) {
          throw new Error(
            payload.message || "Failed to spin Chaos Draft packs.",
          );
        }
        ...
        if (
          !currentSpinResult ||
          !currentSpinResult.winning_pack ||
          !currentSpinResult.chosen_variant
        ) {
        ...
        idleCta.classList.remove("chaos-draft-idle-cta-sinking");
        message.textContent =
          error.message || "Failed to spin Chaos Draft packs.";
        setButtonsForIdle();
      }
    }, 180);
  }
  ...
  if (inlineViewUpscaleButton) {
    inlineViewUpscaleButton.addEventListener("click", function () {
      const cardUuids = getInlinePackBatchUpscaleCardUuids();

      if (!cardUuids.length) {
        window.alert("This pack does not contain any cards to upscale.");
        return;
      }
      ...
      if (
        ...
        window.alert("Batch Upscale controls are not available.");
        return;
      }

      const packDisplayName =
        String(
          inlineViewTitle ? inlineViewTitle.textContent : "this pack",
        ).trim() || "this pack";

      window.iMomirUpscaleBatch.requestCards(cardUuids, {
        title: "Batch Upscale Pack",
        sourceLabel: packDisplayName,
        message:
          "Upscale all " +
          cardUuids.length +
          ...
          packDisplayName +
          "?",
        hidePageRefresh: true,
        onComplete: function () {
          refreshInlinePackUpscaleResults().catch(function (error) {
            console.error(
              "Could not refresh Upscaled card images after Batch Upscale.",
              error,
            );
          });
        },
  ...
  document.addEventListener("imomir:card-image-refreshed", function (event) {
    ...
    const cardUuid = String(detail.cardUuid || "").trim();

    if (!cardUuid || !detail.imageChanged) {
      return;
    }
  ...
  if (packZoomOverlay) {
    packZoomOverlay.addEventListener("click", function (event) {
      if (
        ...
        event.target.closest(".chaos-pack-inline-flip-button")
      ) {
        return;
      }
